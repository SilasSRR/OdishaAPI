...
async function searchVideos(afterDate) {
...
async function main() {
  ...
  let state = await SyncState.findOne({ key: "other" });

  if (!state) {
    state = await SyncState.create({ key: "other", lastSyncedAt: null });
  }
  ...
  console.log("Fetching Other videos after:", after.toISOString());

  const ids = await searchVideos(after);

  console.log("Found", ids.length, "recent Other uploads");
  ...
  console.log("Other incremental sync complete");
