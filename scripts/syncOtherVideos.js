...
if (!API_KEY || !MONGODB_URI || !OTHER_CHANNEL_ID) {
  console.error("Missing env vars: YT_API_KEY, MONGODB_URI/MONGO_URI, YT_OTHER_CHANNEL_ID");
  process.exit(1);
}
...
async function searchItemsByDateRange({ channelId, start, end }) {
  const items = [];
  let pageToken = "";
...
async function syncYear(year) {
  ...
  console.log(`Sync Other videos for ${year}`);

  const ids = await searchItemsByDateRange({
    channelId: OTHER_CHANNEL_ID,
    start,
    end,
  });

  console.log(`Found ${ids.length} Other video ids for ${year}`);
...
async function main() {
  ...
  console.log("Other sync complete");
}

main().catch(async (e) => {
  console.error("Other sync failed:", e);
  try {
    await mongoose.disconnect();
  } catch {}
  process.exit(1);
